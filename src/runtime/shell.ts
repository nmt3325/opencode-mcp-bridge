...
export const ShellTool = Tool.define("bash", Effect.succeed({
  description: "Execute a shell command in the workspace (bash on POSIX, cmd.exe on Windows). The timeout is in milliseconds (default 120000). Output is bounded and longer output is saved for read. Commands require permission by default. A running job is not completion; poll its job_id, never repeat it. This tool does not invoke an AI model.",
