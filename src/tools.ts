...
import { clamp, fail, handler, ok, sleep, withCap, type ToolResult } from "./result.js"
import { registerModelTools } from "./modelTools.js"

export { HARD_CAP_MS, sleep } from "./result.js"
...
export function registerTools(server: McpServer, client: OpencodeClient, config: BridgeConfig): void {
	// opencode's own model facing tools (read/write/edit/bash/glob/grep/...),
	// mirrored so an MCP client can be the model. See modelTools.ts.
	registerModelTools(server, client, config)
