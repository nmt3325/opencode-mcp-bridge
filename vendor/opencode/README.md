...
- The execution-only `Tool.define` supplies schema validation, permission callbacks that accept the capabilities these tools implement and reject anything else as unsupported, abort signals and output bounds. Node filesystem adapters provide only the methods consumed by the extracted leaves, plus path canonicalization, private-directory guards and per-file atomic/conflict-checked writes.
...
- The shell uses Node process groups and private environment/state rather than OpenCode's tree-sitter/plugin/config/process service graph. Schema and output algorithms are extracted; shell parsing and permission handling are explicitly different: there is no interactive approval flow and no permission policy, as documented in the main README.
