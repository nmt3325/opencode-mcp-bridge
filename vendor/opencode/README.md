...
- The execution-only `Tool.define` supplies schema validation, permission callbacks, abort signals and output bounds. Node filesystem adapters provide only the methods consumed by the extracted leaves, plus path guards and per-file atomic/conflict-checked writes.
...
- The shell uses Node process groups and private environment/state rather than OpenCode's tree-sitter/plugin/config/process service graph. Schema and output algorithms are extracted; shell parsing and permissions are explicitly different and documented in the main README.
- `apply_patch` uses the upstream pure parser/deriver with a new approval/workspace adapter. Existing add/move destinations and duplicate targets are rejected. It is atomic per file, not a multi-file transaction.
